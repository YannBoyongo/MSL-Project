    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            @can('countries.manage')
                <a href="{{ route('pahewo.languages.create') }}" class="inline-flex items-center rounded-md bg-gray-800 px-4 py-2 text-sm text-white">Nouvelle langue</a>
            @endcan
        </div>
    </x-slot>
                        ...
                        <tbody>
                            @forelse ($languages as $language)
                                <tr class="border-b">
                                    ...
                                    <td class="py-2">
                                        @can('countries.manage')
                                            <a href="{{ route('pahewo.languages.edit', $language) }}" class="text-indigo-600 hover:underline">Modifier</a>
                                        @endcan
                                    </td>
                                </tr>
                            @empty
