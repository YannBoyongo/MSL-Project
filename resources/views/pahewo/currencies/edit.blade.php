    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="rounded-lg bg-white p-6 shadow-sm">
                ...
                <form method="POST" action="{{ route('pahewo.currencies.update', $currency) }}" class="mt-6 space-y-6">
                    @csrf
                    @method('PUT')
                    @include('pahewo.currencies._form', ['currency' => $currency])
                    <div class="flex gap-3">
                        <x-primary-button>Mettre à jour</x-primary-button>
                        <a href="{{ route('pahewo.currencies.index') }}" class="inline-flex items-center rounded-md border px-4 py-2 text-sm">Annuler</a>
                    </div>
                </form>
            </div>
        </div>
