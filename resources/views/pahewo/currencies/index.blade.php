    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            @can('countries.manage')
                <a href="{{ route('pahewo.currencies.create') }}" class="inline-flex items-center rounded-md bg-gray-800 px-4 py-2 text-sm text-white">Nouvelle devise</a>
            @endcan
        </div>
    </x-slot>
                        ...
                        <tbody>
                            @forelse ($currencies as $currency)
                                <tr class="border-b">
                                    ...
                                    <td class="py-2">
                                        @can('countries.manage')
                                            <a href="{{ route('pahewo.currencies.edit', $currency) }}" class="text-indigo-600 hover:underline">Modifier</a>
                                        @endcan
                                    </td>
                                </tr>
                            @empty
