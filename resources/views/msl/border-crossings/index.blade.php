    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            @can('travel_documents.manage')
                <a href="{{ route('msl.border-crossings.create') }}" class="inline-flex items-center rounded-md bg-gray-800 px-4 py-2 text-sm text-white">Nouveau poste</a>
            @endcan
        </div>
    </x-slot>
                        ...
                        <tbody>
                            @forelse ($borderCrossings as $crossing)
                                <tr class="border-b">
                                    ...
                                    <td class="py-2">
                                        @can('travel_documents.manage')
                                            <a href="{{ route('msl.border-crossings.edit', $crossing) }}" class="text-indigo-600 hover:underline">Modifier</a>
                                        @endcan
                                    </td>
                                </tr>
                            @empty
