    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="rounded-lg bg-white p-6 shadow-sm">
                ...
                <form method="POST" action="{{ route('msl.claim-types.update', $claimType) }}" class="mt-6 space-y-6">
                    @csrf
                    @method('PUT')
                    @include('msl.claim-types._form', ['claimType' => $claimType, 'translation' => $translation])
                    <div class="flex gap-3">
                        <x-primary-button>Mettre à jour</x-primary-button>
                        <a href="{{ route('msl.claim-types.index') }}" class="inline-flex items-center rounded-md border px-4 py-2 text-sm">Annuler</a>
                    </div>
                </form>
            </div>
        </div>
