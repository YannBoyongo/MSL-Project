...
class PahewoFilterTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_country_filter_can_be_cleared(): void
    {
        ...
        $this->actingAs($user)
            ->get(route('pahewo.markets.index', ['country_id' => $country->id]))
            ->assertOk();

        $this->actingAs($user)
            ->get(route('pahewo.markets.index', ['country_id' => '']))
            ->assertOk();
    }
    ...
    public function test_search_filter_preserves_country_filter(): void
    {
        ...
        $this->actingAs($user)
            ->get(route('pahewo.markets.index', ['country_id' => $country->id, 'search' => 'Central']))
            ->assertOk()
    ...
    public function test_commodity_prices_default_to_today(): void
    {
        ...
        $this->actingAs($user)
            ->get(route('pahewo.commodity-prices.index'))
            ->assertOk()
    ...
    public function test_commodity_prices_can_show_all_dates(): void
    {
        ...
        $this->actingAs($user)
            ->get(route('pahewo.commodity-prices.index', ['price_date' => '']))
            ->assertOk();
    }
}
